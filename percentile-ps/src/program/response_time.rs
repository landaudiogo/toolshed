...
use std::{collections::BTreeMap, fs::File, io::{self, stdout, Write}, path::{PathBuf}};

pub struct PercentileResponseTime {}

impl PercentileResponseTime {
    pub fn run(time_factor: u64, percentile: f64, output_file: Option<PathBuf>) {
        ...
        for current_epoch in (*start_epoch..=*end_epoch).step_by(time_factor as usize) {
            ...
            writeln!(buf, "{:?},{:?}", current_epoch / time_factor, qv as f64 / time_factor as f64);
        }
    }
}
